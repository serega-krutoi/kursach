...
// ============================================================================
//                  ОСНОВНОЙ ГРАФОВЫЙ ГЕНЕРАТОР (generateSchedule)
// ============================================================================
...
// ============================================================================
//                  ПРОСТОЙ ЖАДНЫЙ ГЕНЕРАТОР (generateScheduleSimple)
// ============================================================================

std::vector<ExamAssignment> generateScheduleSimple(
    const std::vector<Exam>& exams,
    const std::vector<Group>& groups,
    const std::vector<Subject>& subjects,   // не используем, для совместимости
    const std::vector<Timeslot>& timeslots,
    const std::vector<Room>& rooms,
    int maxExamsPerDayForGroup
) {
    (void)subjects; // чтобы не ругался компилятор
    ...
    if (timeslots.empty()) {
        logWarning("Список таймслотов пуст. Простое расписание не будет создано.");
        return assignments;
    }

    // Контроль занятости:
    // (groupId, timeslotId)   -> группа занята
    // (teacherId, timeslotId) -> препод занят
    ...
    for (int examIndex = 0; examIndex < (int)exams.size(); ++examIndex) {
        ...
        // Перебираем слоты
        for (int tsOrderIndex = 0;
             tsOrderIndex < (int)timeslotOrder.size() && !placed;
             ++tsOrderIndex)
        {
            ...
            // Сначала проверяем ограничение "максимум экзаменов в день" для группы
            if (!canPlaceExamForGroupOnDate(
                    exam,
                    examIndex,
                    timeslotId,
            ...
            // группа или препод уже заняты в этот слот
        ...
        if (!placed) {
            logError("SIMPLE: не удалось назначить exam id=" +
                     std::to_string(exam.id) +
                     " (groupId=" + std::to_string(groupId) + ")");
        }
        ...
        ExamAssignment a;
        a.examIndex  = examIndex;
        a.timeslotId = chosenTimeslotId; // может быть -1
        a.roomId     = chosenRoomId;     // может быть -1
